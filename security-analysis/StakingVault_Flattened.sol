...
// File contracts_active/StakingVault.sol
...
contract StakingVault is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Stake AIPG tokens to earn rewards
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        // Claim existing rewards for this user
    ...
    function pendingRewards(address user) external view returns (uint256) {
        ...
        uint256 userReward = (stakedAmount[user] * rewardRate * timeElapsed) / 1e18;
        
        return userRewards[user] + userReward;
    }
    
    /**
     * @dev Called by EmissionsController to distribute rewards
     * @param amount Total reward amount to distribute
     */
    function onReward(address /* worker */, uint256 amount) external onlyOwner {
    ...
    /**
     * @dev Emergency function to pause staking (only owner)
     */
    function emergencyPause() external onlyOwner {
        // This could be expanded with a pause mechanism
        // For now, just a placeholder for emergency controls
    }
    ...
    function _updateRewards() internal {
        ...
        if (timeElapsed > 0) {
            uint256 totalReward = (totalStaked * rewardRate * timeElapsed) / 1e18;
            if (totalReward > 0) {
                // Distribute rewards proportionally to all stakers
                // This is a simplified version - in practice you might want more sophisticated distribution
                rewardRate = (rewardRate * totalStaked) / (totalStaked + totalReward);
            }
            lastUpdateTime = block.timestamp;
        }
    }
    ...
    function _claimRewards(address user) internal {
        uint256 pending = userRewards[user];
        if (pending > 0) {
            userRewards[user] = 0;
            lastRewardTime[user] = block.timestamp;
            
            // Mint tokens to user (requires EmissionsController to have MINTER_ROLE)
            // Note: This assumes the StakingVault has MINTER_ROLE on the token
            // Alternative: transfer from EmissionsController balance
     ...
     * @return pending User's pending rewards
     * @return totalStakedInVault Total amount staked in vault
     */
    function getUserInfo(address user) external view returns (
        ...
        totalStakedInVault = totalStaked;
    }
    
    /**
     * @dev Get vault statistics
     * @return totalStakedTokens Total tokens staked
     * @return totalRewardsGiven Total rewards distributed
