...
abstract contract DiamondHarness is Test {
    ...
    function _selectorsWorkerRegistry() private pure returns (bytes4[] memory s) {
        ...
        s[6] = WorkerRegistry.getWorkerAt.selector;
        s[7] = WorkerRegistry.getMinBond.selector;
        s[8] = WorkerRegistry.setMinBond.selector;
        s[9] = WorkerRegistry.cancelUnbond.selector;
        s[10] = WorkerRegistry.withdrawBond.selector;
        s[11] = WorkerRegistry.slash.selector;
        s[12] = WorkerRegistry.setUnbondingPeriod.selector;
        s[13] = WorkerRegistry.unbondingPeriod.selector;
        s[14] = WorkerRegistry.getUnbondInfo.selector;
        s[15] = WorkerRegistry.isSlashEvidenceUsed.selector;
    }
