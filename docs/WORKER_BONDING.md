...
1. **Cut the facet:** `scripts/deployment/deploy-worker-bonding-facet.sh`
   — deploys the new WorkerRegistry impl and `updateModules`: REPLACEs the live
   selectors (so `unbond()` gets the safe cooldown behavior + `getWorker` returns
   the 8-field struct) and ADDs the new ones. The script classifies each selector
   at runtime via `moduleAddress(bytes4)`, so it's correct regardless of which
   selectors were live.
